	...
	AutoDust.Sleep(0.1, "updaterate");
		...
		for i, alert in ipairs(AutoDust.Unique) do
			...
			for i = 1, Heroes.Count() do
			    ...
			    if TA and NPC.IsEntityInRange(hero, TAhero, 1000) and (Entity.GetHealth(TAhero)/Entity.GetMaxHealth(TAhero)<0.3) and not NPC.HasModifier(TAhero, "modifier_truesight") and not Entity.IsSameTeam(hero, TAhero) and not NPC.HasState(hero, Enum.ModifierState.MODIFIER_STATE_INVISIBLE) then
			    Ability.CastNoTarget(dust)	
			    AutoDust.Sleep(0.1, "updaterate");
			    end
			end	
